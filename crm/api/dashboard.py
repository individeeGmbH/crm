...
@frappe.whitelist()
def reset_to_default(dashboard_name: str = "Manager Dashboard"):
...
def get_dashboard(from_date: str | None = None, to_date: str | None = None, user: str | None = None,
                  dashboard_name: str = "Manager Dashboard"):
    """
    ...
    if not frappe.db.exists("CRM Dashboard", dashboard_name):
        if dashboard_name == "Manager Dashboard":
...
def get_total_calls(from_date=None, to_date=None, user=None):
...
def get_calls(title, tooltip, from_date=None, to_date=None, user=None, status=None, min_duration=None):
    ...
    # Build conditions for current period
    current_cond = (CallLog.creation >= from_date) & (CallLog.creation < to_date_plus_one)
    # Build conditions for previous period
    prev_cond = (CallLog.creation >= prev_from_date) & (CallLog.creation < from_date)

    if user:
        current_cond = current_cond & (CallLog.caller == user)
        prev_cond = prev_cond & (CallLog.caller == user)
    if status:
        current_cond = current_cond & (CallLog.status == status)
        prev_cond = prev_cond & (CallLog.status == status)
    if min_duration:
        current_cond = current_cond & (CallLog.duration > min_duration)
        prev_cond = prev_cond & (CallLog.duration > min_duration)

    # Build query with CASE expressions
    query = frappe.qb.from_(CallLog).select(
        Count(Case().when(current_cond, CallLog.name).else_(None)).as_("current_period_calls"),
        Count(Case().when(prev_cond, CallLog.name).else_(None)).as_("prev_period_calls"),
    )

    result = query.run(as_dict=True)

    current_period_calls = result[0].current_period_calls or 0
    prev_period_calls = result[0].prev_period_calls or 0

    delta_in_percentage = (
        (current_period_calls - prev_period_calls) / prev_period_calls * 100 if prev_period_calls else 0
    )

    return {
        "title": title,
        "tooltip": tooltip,
        "value": current_period_calls,
        "delta": delta_in_percentage,
        "deltaSuffix": "%",
    }


def get_converted_leads(from_date=None, to_date=None, user=None):
    """
    Count leads converted into deals within the period.
    A conversion is recorded as a CRM Deal with a non-empty 'lead' field.
    """
    if not from_date or not to_date:
        from_date = frappe.utils.get_first_day(from_date or frappe.utils.nowdate())
        to_date = frappe.utils.get_last_day(to_date or frappe.utils.nowdate())
    ...
    result = (
        ...
        .select(
            ...
            Count(Case().when(prev_cond, Deal.name).else_(None)).as_("previous"),
        )
        .run(as_dict=True)
    )

    current = result[0].current or 0
    previous = result[0].previous or 0
    delta = (current - previous) / previous * 100 if previous else 0

    return {
        "title": _("Converted Leads"),
...
def get_calls_rejected_time(from_date=None, to_date=None, user=None):
    return get_calls_for_rejection(_("Rejected: No Time"), _("Calls rejected due to no time in this period"), "No Time",
                                   from_date, to_date, user)


def get_calls_rejected_budget(from_date=None, to_date=None, user=None):
    return get_calls_for_rejection(_("Rejected: Budget"), _("Calls rejected due to budget in this period"),
                                   "Budget Constraints",
                                   from_date, to_date, user)
...
def get_calls_for_rejection(title, tooltip, reason, from_date=None, to_date=None, user=None):
    """
    Count calls where the rejection reason is 'Budget'.
    """
    diff = frappe.utils.date_diff(to_date, from_date)
    if diff == 0:
        diff = 1
    ...
    current_cond = (
            (CallLog.creation >= from_date)
            & (CallLog.creation < to_date_plus_one)
            & (CallLog.rejection_reason == reason)
    )
    prev_cond = (
            (CallLog.creation >= prev_from_date)
            & (CallLog.creation < from_date)
            & (CallLog.rejection_reason == reason)
    )
    ...
    result = (
        frappe.qb.from_(CallLog)
        .select(
            Count(Case().when(current_cond, CallLog.name).else_(None)).as_("current"),
            Count(Case().when(prev_cond, CallLog.name).else_(None)).as_("previous"),
        )
        .run(as_dict=True)
    )
