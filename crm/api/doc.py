...
@frappe.whitelist()
def get_quick_filters(doctype: str, view_name: str | None, cached: bool = True):
    meta = frappe.get_meta(doctype, cached)
    fields = []

    # View-specific quick filters take priority
    if view_name and frappe.db.exists("CRM View Settings", view_name):
        view_filters = frappe.db.get_value("CRM View Settings", view_name, "quick_filters")
        if view_filters:
            _filters = json.loads(view_filters)
            for f in _filters:
                if f == "name":
                    fields.append({"label": "Name", "fieldname": "name", "fieldtype": "Data"})
                else:
                    field = next((m for m in meta.fields if m.fieldname == f), None)
                    if field:
                        fields.append(field)
            return _format_quick_filters(fields, doctype)
...
@frappe.whitelist()
def update_quick_filters(quick_filters: str, old_filters: str, doctype: str, view_name: str | None):
    quick_filters = json.loads(quick_filters)
    old_filters = json.loads(old_filters)

    if view_name:
        # Save to the view record only — don't touch in_standard_filter
        frappe.db.set_value(
            "CRM View Settings", view_name, "quick_filters", json.dumps(quick_filters)
        )
        return
...
def _format_quick_filters(fields, doctype):
    quick_filters = []
    for field in fields:
